...
def _build_transforms():
    ...
    # NOT variants
    for k in range(1, 8):
        transforms.append((f"NOT ROT({k})", lambda v, k=k: _rotate_left(v, k) ^ 0xFF))
    for k in range(1, 8):
        transforms.append((f"NOT SHL({k})", lambda v, k=k: _shift_left(v, k) ^ 0xFF))
    for k in range(1, 8):
        transforms.append((f"NOT SHR({k})", lambda v, k=k: _shift_right(v, k) ^ 0xFF))
    return transforms


TRANSFORMS = _build_transforms()
TRANSFORM_DICT = {name: fn for name, fn in TRANSFORMS}


def solve_extended(inputs_str: list[str], outputs_str: list[str], query_str: str):
    inputs = [int(x, 2) for x in inputs_str]
    outputs = [int(x, 2) for x in outputs_str]
    query = int(query_str, 2)
    n = len(inputs)
    nt = len(TRANSFORMS)
    ...
    # 1. Try Majority(T1, T2, T3)
    for t1 in range(nt):
        for t2 in range(t1 + 1, nt):
            a_and_b = [results[t1][i] & results[t2][i] for i in range(n)]
            for t3 in range(t2 + 1, nt):
                t1_or_t2 = [results[t1][i] | results[t2][i] for i in range(n)]
                if all(
                    (a_and_b[i] | (results[t3][i] & t1_or_t2[i])) == outputs[i]
                    for i in range(n)
                ):
                    desc = f"MAJ({TRANSFORMS[t1][0]},{TRANSFORMS[t2][0]},{TRANSFORMS[t3][0]})"
                    ans_t1 = TRANSFORMS[t1][1](query)
                    ans_t2 = TRANSFORMS[t2][1](query)
                    ans_t3 = TRANSFORMS[t3][1](query)
                    ans = (ans_t1 & ans_t2) | (ans_t3 & (ans_t1 | ans_t2))
                    return format(ans, "08b"), desc, ("MAJ", TRANSFORMS[t1][0], TRANSFORMS[t2][0], TRANSFORMS[t3][0])

    # 2. Try Choice(T1, T2, T3)
    for t1 in range(nt):
        not_t1 = [r ^ 0xFF for r in results[t1]]
        for t2 in range(nt):
            if t2 == t1:
                continue
            for t3 in range(t2 + 1, nt):
                if t3 == t1:
                    continue
                if all(
                    ((results[t1][i] & results[t2][i]) | (not_t1[i] & results[t3][i])) == outputs[i]
                    for i in range(n)
                ):
                    desc = f"CH({TRANSFORMS[t1][0]},{TRANSFORMS[t2][0]},{TRANSFORMS[t3][0]})"
                    ans_t1 = TRANSFORMS[t1][1](query)
                    ans_t2 = TRANSFORMS[t2][1](query)
                    ans_t3 = TRANSFORMS[t3][1](query)
                    ans = (ans_t1 & ans_t2) | ((ans_t1 ^ 0xFF) & ans_t3)
                    return format(ans, "08b"), desc, ("CH", TRANSFORMS[t1][0], TRANSFORMS[t2][0], TRANSFORMS[t3][0])
...
def _evaluate_rule(bits: str, rule: RuleCandidate, bit_idx: Optional[int] = None) -> str:
    ...
    if rule.family == "MAJ":
        assert bit_idx is not None
        parts = rule.expr[4:-1].split(",")
        t1, t2, t3 = parts[0], parts[1], parts[2]
        val1 = (TRANSFORM_DICT[t1](int(bits, 2)) >> bit_idx) & 1
        val2 = (TRANSFORM_DICT[t2](int(bits, 2)) >> bit_idx) & 1
        val3 = (TRANSFORM_DICT[t3](int(bits, 2)) >> bit_idx) & 1
        res = (val1 & val2) | (val2 & val3) | (val3 & val1)
        return str(res)
    if rule.family == "CH":
        assert bit_idx is not None
        parts = rule.expr[3:-1].split(",")
        t1, t2, t3 = parts[0], parts[1], parts[2]
        val1 = (TRANSFORM_DICT[t1](int(bits, 2)) >> bit_idx) & 1
        val2 = (TRANSFORM_DICT[t2](int(bits, 2)) >> bit_idx) & 1
        val3 = (TRANSFORM_DICT[t3](int(bits, 2)) >> bit_idx) & 1
        res = (val1 & val2) | ((val1 ^ 1) & val3)
        return str(res)
...
def _emit_apply(
    ...
    for i, rule in enumerate(vector):
        ...
        if rule.family == "MAJ":
            parts = rule.expr[4:-1].split(",")
            t1, t2, t3 = parts[0], parts[1], parts[2]
            val1 = (TRANSFORM_DICT[t1](int(question_bits, 2)) >> i) & 1
            val2 = (TRANSFORM_DICT[t2](int(question_bits, 2)) >> i) & 1
            val3 = (TRANSFORM_DICT[t3](int(question_bits, 2)) >> i) & 1
            result = str((val1 & val2) | (val2 & val3) | (val3 & val1))
            lines.append(f"{i} {rule.expr} = MAJ({val1},{val2},{val3}) = {result}")
        ...
        if rule.family == "CH":
            ...
            val2 = (TRANSFORM_DICT[t2](int(question_bits, 2)) >> i) & 1
            val3 = (TRANSFORM_DICT[t3](int(question_bits, 2)) >> i) & 1
            result = str((val1 & val2) | ((val1 ^ 1) & val3))
            lines.append(f"{i} {rule.expr} = CH({val1},{val2},{val3}) = {result}")
            answer_bits.append(result)
            continue
        ...
        if rule.family in SYM_FAMILIES:
            result = _evaluate_rule(question_bits, rule, i)
...
def reasoning_bit_manipulation(problem: Problem) -> Optional[str]:
    ...
    pred_ans, rule_desc, rule_info = solve_extended(inputs, outputs, question_bits)
    ...
    if rule_info is not None:
        lines.append(f"Found global {rule_info[0]} rule: {rule_desc}")
        lines.append("")
        family, t1, t2, t3 = rule_info
        best = [
            RuleCandidate(family, None, None, f"{family}({t1},{t2},{t3})")
